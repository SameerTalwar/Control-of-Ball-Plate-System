...
[t,x]=ode45(@(t,x) ballplate1d(t,x,desired,simspan),simspan,init,opt);
...
for ii=1:numel(t)
    ...
    [~,uinstx(ii),uinsty(ii)]=ballplate1d(t(ii),x(ii,:),desired,simspan);
end
%%
figure(1)
plot(t,x(:,1),'--',t,desiredx,'-.')
title('Tracking error of system using sliding mode controller')
